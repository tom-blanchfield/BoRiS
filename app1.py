...
if selection_type == "Genres":
    ...
    if len(selected_authors) > 0:
        st.title("Here are your author based recommendations:")
        ...
        for book_id in top_raters_ratings.columns:
            if len(user_ratings) >= 5:  # Limit user ratings to 5
                break
            if books.loc[books['goodreads_book_id'] == book_id, 'authors'].values[0].split(',')[0].strip() in selected_authors:
                user_ratings = user_ratings.append({'book_id': book_id, 'user_id': 'user_id', 'rating': 5}, ignore_index=True)

        included_books = set(user_ratings['book_id'].tolist())

        # Calculate cosine similarities between the user and top raters
        user_similarities = cosine_similarity(top_raters_ratings.append(user_ratings).fillna(0))[0]
